...
パッケージマネージャは **npm** です。デプロイ先のロリポップ！デプロイナウが npm のみ対応のため、pnpm / yarn を使ってはなりません。
...
データベース・認証・ランキング・投稿機能は**将来も含めて作りません**。全ルートをビルド時に静的生成し、抽選結果とコレクションはクライアント側（localStorage）だけで扱います。この前提を崩す提案をしてはなりません。
...
- ブランチ名は `{prefix}/{kebab-case の要約}` とします。例: `feat/draw-logic`
