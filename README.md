...
See `.env.example` for the environment variables. `NEXT_PUBLIC_*` values are inlined at build time, so set them on the host before the build runs. Analytics stays off unless `NEXT_PUBLIC_GA_ID` is set, never sends hits from `localhost`, and stores nothing in cookies until the visitor accepts the consent banner. See `/privacy` for what is collected. Without `NEXT_PUBLIC_GA_ID` there is nothing to consent to, so the banner and the consent controls on `/privacy` are not rendered — set the variable in `.env.local` to work on them.
